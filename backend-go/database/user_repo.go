...
func (s *Store) InsertUser(email, passwordHash, code string, expiresAt time.Time) error {
	_, err := s.DB.Exec(`
...
func (s *Store) InsertUserVerified(email, passwordHash string) error {
	_, err := s.DB.Exec(`
...
func (s *Store) FindUserByID(userID int) (*models.User, error) {
	var u models.User
	row := s.DB.QueryRow(`
...
func (s *Store) UpdateVerificationCode(email, code string, expiresAt time.Time) error {
	_, err := s.DB.Exec(`
...
func (s *Store) VerifyUser(email string) error {
	_, err := s.DB.Exec(`
...
func (s *Store) GetAllUsers() ([]models.User, error) {
	...
	for rows.Next() {
		var u models.User
		if err := rows.Scan(&u.ID, &u.Email, &u.Role, &u.IsVerified, &u.CreatedAt); err != nil {
			return nil, err
		}
...
func (s *Store) DeleteUnverifiedUsers() error {
	_, err := s.DB.Exec(`DELETE FROM users WHERE is_verified = false`)
	return err
}
