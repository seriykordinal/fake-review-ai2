...
const refreshUsersBtn = getElement('refreshUsers');
const refreshProductsBtn = getElement('refreshProducts');
const backToSiteBtn = getElement('backToSite');
const logoutAdminBtn = getElement('logoutBtn');
...
function renderUserActions(user) {
    if (currentUserRole === 'super_admin') {
        if (user.role === 'super_admin')
            return 'Супер-админ (нельзя изменять)';
        return `
...
function attachUserEvents() {
    document.querySelectorAll('[data-action="deleteUser"]').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            if (!id || !confirm('Удалить пользователя? Это также удалит все его анализы.'))
                return;
            try {
    ...
    document.querySelectorAll('[data-action="changeRole"]').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            const email = btn.closest('tr')?.cells[1]?.innerText;
            if (!email)
                return;
            try {
...
function attachProductEvents() {
    document.querySelectorAll('[data-action="deleteProduct"]').forEach(btn => {
        btn.addEventListener('click', async () => {
            const id = btn.dataset.id;
            if (!id || !confirm('Удалить анализ товара? Все отзывы также будут удалены.'))
                return;
            try {
