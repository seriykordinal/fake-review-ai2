...
qs('analyzeBtn').addEventListener('click', async () => {
    ...
    if (!text) {
        toast('Введите текст отзыва', 'err');
        return;
    }
...
qs('analyzeProductBtn').addEventListener('click', async () => {
    if (isAnalyzing) {
        toast('Анализ уже выполняется...');
        return;
    }
    const url = qs('productUrl').value.trim();
    if (!url) {
        toast('Введите ссылку на товар', 'err');
        return;
    }
    if (!token) {
        toast('Необходима авторизация', 'err');
        return;
    }
    isAnalyzing = true;
    const btn = qs('analyzeProductBtn');
    btn.disabled = true;
...
async function loadHistory() {
    ...
    try {
        const data = await api.get('/api/history', token);
        if (!data.length) {
        ...
        data.forEach((item, i) => {
            ...
            el.addEventListener('click', () => {
                qs('productUrl').value = item.product_url;
                showScreen('auth');
            });
            list.appendChild(el);
        });
        ...
        list.innerHTML = `<div class="history-empty">Ошибка: ${escHtml(e.message)}</div>`;
    }
}
// ===== Init =====
async function init() {
